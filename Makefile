...
build: $(TARGET) $(ENV) dependencies
	@mkdir -p $(TARGET)/
	@cp -frv ./*.html ./bower.json ./demo ./sets ./README.md $(TARGET)/;
	@echo "Project build complete."

ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-icons" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-icons"
endif
